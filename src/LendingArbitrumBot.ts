...
import {
  ...
  getPastEvents,
} from './utils/web3Calls/generic.js';
import { telegramBotMain } from './utils/telegram/TelegramBot.js';
import { EventEmitter } from 'events';
import {
  handleLiveEvents,
  manageMarket,
  watchingForNewMarketOpenings,
} from './utils/Oragnizer.js';
...
export const LENDING_MIN_LOAN_CHANGE_AMOUNT_WORTH_PRINTING = 1000;
export const LENDING_MIN_LIQUIDATION_DISCOUNT_WORTH_PRINTING = 10;
...
async function launchClassicCrvUSDMonitoring() {
  const WEB3_WS_PROVIDER =
    getWeb3WsProvider();

  const crvUSD_ControllerFactory =
    new WEB3_WS_PROVIDER.eth.Contract(
  ...
  if (
    !(
      ADDED_MARKET_EVENTS instanceof
      Array
    )
  )
    return;
  ...
  await watchingForNewMarketOpenings(
    crvUSD_ControllerFactory,
    eventEmitter
  );
  await handleLiveEvents(eventEmitter);
  console.log(
    'crvUSD_Bot launched successfully.'
  );
}

async function main() {
  await telegramBotMain(
    ENV,
    eventEmitter
  );
