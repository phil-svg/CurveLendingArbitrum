...
import { getPastEvents, subscribeToPegkeeperEvents, web3Call } from '../../web3/generic.js';
...
async function getAllPegKeepersInfo(blockNumber) {
    ...
    for (const pegKeeperAddress of pegkeeperAddressArrOnchain) {
        const PEG_KEEPER_CONTRACT = new web3WsProvider.eth.Contract(ABI_Pegkeeper, pegKeeperAddress);
        const pool = await web3Call(PEG_KEEPER_CONTRACT, 'pool', []);
        const coinAddress = await getPegkeeperCoin(pool, web3WsProvider);
        const coinSymbol = coinAddress ? await getCoinSymbol(coinAddress, web3WsProvider) : null;
...
export async function calculateDebtsForAllPegKeepers(blockNumber, allPegKeepersInfo) {
    ...
    for (const pegKeeperInfo of allPegKeepersInfo) {
        const debts = await getPegKeeperDebtAtBlocks(pegKeeperInfo, blockNumber, web3WsProvider);
...
export async function livemodePegKeepers(blockNumber, eventEmitter) {
    ...
    for (const address of pegkeeperAddressArrOnchain) {
        const PEG_KEEPER_CONTRACT = new web3WsProvider.eth.Contract(ABI_Pegkeeper, address);
...
export async function pegkeeperHisto(eventEmitter, startBlock, endBlock) {
    ...
    const allHistoEvents = await getAllEvents(web3WsProvider, startBlock, endBlock);
